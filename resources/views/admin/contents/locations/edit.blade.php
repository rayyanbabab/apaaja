...
html.dark .loc-icon-wrap { background-color: rgba(245,158,11,0.2) !important; }
...
<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-5">
    ...
    <div class="flex items-center justify-between">
        <div>
            <div class="loc-breadcrumb flex items-center gap-2 text-xs text-gray-400 mb-1">
                ...
                <span class="text-gray-600 font-medium">Edit Lokasi</span>
            </div>
            <h1 class="loc-title text-xl font-bold text-gray-900">Edit Lokasi</h1>
            <p class="loc-subtitle text-sm text-gray-400 mt-0.5">Perbarui informasi lokasi "{{ $location->name }}"</p>
        </div>
    ...
    <div class="loc-card bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div class="loc-card-header px-6 py-4 border-b border-gray-100 bg-gray-50/50 flex items-center gap-3">
            <div class="loc-icon-wrap w-9 h-9 bg-amber-50 rounded-xl flex items-center justify-center">
            ...
            <div>
                <p class="loc-card-header-title text-sm font-semibold text-gray-900">Edit Informasi Lokasi</p>
                <p class="loc-card-header-sub text-xs text-gray-400">Perbarui detail informasi lokasi penyimpanan</p>
            </div>
        </div>

        <form id="update-form-{{ $location->id }}" method="POST" action="{{ route($routePrefix . '.locations.update', $location) }}">
            @csrf
            @method('PUT')
            ...
            <div class="px-6 py-5 space-y-5">
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="relative">
                        <input type="text" id="kode" name="kode" value="{{ old('kode', $location->kode) }}"
                               placeholder="cth: LSV"
                               maxlength="20"
                ...
                <div class="space-y-1.5">
                    ...
                    <textarea id="deskripsi" name="deskripsi" rows="3"
                              ...
                              class="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-xl resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition">{{ old('deskripsi', $location->deskripsi) }}</textarea>
                </div>
            ...
            <div class="loc-card-footer flex items-center justify-between px-6 py-4 bg-gray-50 border-t border-gray-100">
                ...
                <button type="button" onclick="openModal('update-modal-{{ $location->id }}')"
...
<script>
    // Edit mode: kode sudah ada, auto-fill OFF kecuali user klik tombol Auto
    let kodeManuallyEdited = true;
...
@push('scripts')
    <x-popup id="update-modal-{{ $location->id }}" title="Konfirmasi Update"
        message="Apakah Anda yakin ingin menyimpan perubahan data lokasi ini?"
        formId="update-form-{{ $location->id }}"
        confirmText="Simpan"
