...
@section('content')
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <div class="px-4 sm:px-6 py-4 border-b border-gray-200">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                    <h1 class="text-xl sm:text-2xl font-bold text-gray-900">Active Borrowings</h1>
                    <p class="text-sm text-gray-600 mt-0.5">List of items currently in active borrowing status</p>
                </div>
                <a href="{{ route($routePrefix . '.borrowings.create') }}" 
                   class="self-start sm:self-auto inline-flex items-center px-3 py-2 sm:px-4 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 focus:bg-blue-700 active:bg-blue-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition ease-in-out duration-150">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div class="px-6 py-4">
            <form method="GET" action="{{ route($routePrefix . '.borrowings.index') }}" class="space-y-4">
                ...
                <div class="flex space-x-2">
                    ...
                    <a href="{{ route($routePrefix . '.borrowings.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-100 hover:bg-gray-200 border border-gray-300 rounded-lg font-medium text-sm text-gray-700 shadow-sm focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-colors duration-200">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <div class="px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-medium text-gray-900">Borrowing Data</h3>
        </div>
                        ...
                        @foreach($borrowings as $borrowing)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                    ...
                                    <button type="button" class="text-green-600 hover:text-green-900" onclick="openModal('ret-bor-{{ $borrowing->id }}')">
                                        Return
                                    </button>
                                    
                                    <a href="{{ route($routePrefix . '.borrowings.edit', $borrowing) }}" class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                    
                                    <button type="button" class="text-red-600 hover:text-red-900" onclick="openModal('del-bor-{{ $borrowing->id }}')">
                                        Delete
                                    </button>
                                </td>
                            </tr>
        ...
        @else
            <div class="px-6 py-12 text-center">
                ...
                <h3 class="mt-2 text-sm font-medium text-gray-900">No items currently borrowed</h3>
                <p class="mt-1 text-sm text-gray-500">There are currently no items in active borrowing status.</p>
                <div class="mt-6">
                    <a href="{{ route($routePrefix . '.borrowings.create') }}" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                        </svg>
                        Add Borrowing
...
@push('scripts')
    @foreach($borrowings as $borrowing)
        <x-popup id="ret-bor-{{ $borrowing->id }}" title="Return Item"
        ...
        <x-popup id="del-bor-{{ $borrowing->id }}" title="Delete Borrowing"
            message="Are you sure you want to delete this data?"
            formId="del-bor-form-{{ $borrowing->id }}"
            confirmText="Delete"
            ...
            cancelText="Cancel" />
        <form id="del-bor-form-{{ $borrowing->id }}" action="{{ route($routePrefix . '.borrowings.destroy', $borrowing) }}" method="POST" style="display: none;">
            @csrf
            @method('DELETE')
        </form>
    @endforeach
