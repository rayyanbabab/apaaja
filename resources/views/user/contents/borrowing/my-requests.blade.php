...
@section('title', 'My Borrowing Requests')
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-8 animate-fade-in">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">My Borrowing Status</h1>
                <p class="text-sm text-gray-600 mt-1">View the status of your current borrowing requests</p>
            </div>
                            ...
                            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                                ...
                                <div class="flex flex-col items-start lg:items-end gap-2.5">
                                    ...
                                    @switch($request->status)
                                        @case('pending')
                                            @if($request->tanggal_pinjam->lt(today()))
                                                {{-- Tanggal pinjam sudah lewat, akan dikancelkan oleh scheduler --}}
                                                <div class="inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-semibold bg-orange-100 text-orange-800 border border-orange-200">
                                                    <svg class="w-3.5 h-3.5 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
                                                    </svg>
                                                    Kedaluwarsa
                                                </div>
                                                <p class="text-xs text-orange-600 font-medium">Tanggal pinjam sudah terlewat</p>
                                            @else
                                                <div class="inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-semibold bg-yellow-100 text-yellow-800 border border-yellow-200">
                                            ...
                                            @break
                                        @case('approved')
                                            @if($request->tanggal_kembali_rencana->isPast())
                                                @php
                                                    $daysLate = (int) ceil($request->tanggal_kembali_rencana->diffInDays(now(), true));
                                                ...
                                                <p class="text-xs text-red-600 font-medium">Terlambat {{ $daysLate }} hari</p>
                                            @else
                                                <div class="inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-semibold bg-green-100 text-green-800 border border-green-200">
                                                    <svg class="w-3.5 h-3.5 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @else
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 py-12">
                <div class="text-center">
                    ...
                    <h3 class="text-lg font-semibold text-gray-900 mb-2">No borrowing requests yet</h3>
                    <p class="text-sm text-gray-500 mb-6">You haven't submitted any borrowing requests yet</p>
                    <a href="{{ route('user.borrowing.index') }}" 
                       class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg shadow-sm hover:shadow transition-all duration-200 gap-2">
                        ...
                        <span>Submit Your First Borrowing Request</span>
                    </a>
                </div>
            </div>
        @endif
